 ...
 */
void generate_splits(char *source, const char *dictionary[], int nwords, char buf[], void *data, void (*process_split)(char buf[], void *data))
{
    int inputTextLength = 0;
    while (source[inputTextLength] != '\0') inputTextLength++;
    ...
    for(int i  = 0;i < nwords;i++){
        int wordLength = 0;
        while (dictionary[i][wordLength] != '\0'){
		wordLength++;
        }
        if(wordLength <= inputTextLength){
            int isMatch = 1;
            ...
            if(isMatch == 1){
                char *new_source = (char*) calloc(inputTextLength-wordLength+1,sizeof(char));
                strncpy(new_source,source+wordLength,inputTextLength-wordLength);
                strcat(new_source,"\0");
                char new_buf[256];
                strcpy(new_buf,buf);
                strcat(new_buf,dictionary[i]);
                if(wordLength < inputTextLength){
                    strcat(new_buf," ");
                }
                generate_splits(new_source,dictionary,nwords,new_buf,data,process_split);
            }
        }
    }
}
